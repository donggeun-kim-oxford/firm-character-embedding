...
# ─── CONFIG ─────────────────────────────────────────────────────────────
STACK_CSV        = "output/embed_port_output/grid_search_results_raw.csv"
MONTHLY_PERF_DIR = "output/embed_port_output/raw_monthly_perf"
PLOTS_DIR        = "output/analysis_output/port_perform_plots_raw"
DESC_STAT_DIR    = "output/analysis_output/descrip_stats_raw"
os.makedirs(PLOTS_DIR, exist_ok=True)
os.makedirs(DESC_STAT_DIR, exist_ok=True)

PORTFOLIOS = ["Benchmark", "rf_raw", "cat_raw", "light_raw"]
...
bench_cols = ['embed_sharpe', 'ew_sharpe', 'rf_raw_sharpe',
              'cat_raw_sharpe', 'light_raw_sharpe']
desc = grid_df_full[bench_cols].describe().T.round(2)
desc.rename(index={
    "embed_sharpe": "Embedding",
    "ew_sharpe":    "Equal-Weight",
    "rf_raw_sharpe":    "Random Forest on Raw Data",
    "cat_raw_sharpe":   "CatBoost on Raw Data",
    "light_raw_sharpe":"LightGBM on Raw Data"
}, inplace=True)
...
out_tex = os.path.join(DESC_STAT_DIR, "desc_stats_raw.tex")
